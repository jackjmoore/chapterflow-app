...
export interface ScrivNode {
  ...
  /** Keyword titles, resolved from the project's keyword table. */
  keywords: string[]
  /** The explicitly-set section type id, if any. Scrivener resolves most
   *  items' type from structural defaults instead; that inheritance is out of
   *  scope, so this is null for the great majority. */
...
/** <Keywords><Keyword>…</Keyword></Keywords> under a item's MetaData. Absent
 *  in the surveyed project, so this is written to tolerate every shape it
 *  might take rather than to match one that was observed. */
function readKeywords(metadata: Element | undefined): string[] {
  if (!metadata) return []
  const block = childNamed(metadata, 'Keywords')
  if (!block) return []
  const out: string[] = []
  for (const child of elements(block)) {
    const text = textOf(child).trim()
    if (text) out.push(text)
  }
  // Some writers put a comma-separated list directly in <Keywords>.
  if (out.length === 0) {
    for (const part of textOf(block).split(',')) {
...
function readItem(element: Element, problems: string[]): ScrivNode | null {
  ...
  const node: ScrivNode = {
    ...
    sectionTypeId: metadata ? textOf(childNamed(metadata, 'SectionType')) || null : null,
    children: []
  }
